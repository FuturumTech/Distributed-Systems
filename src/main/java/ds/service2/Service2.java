...
public class Service2 extends Service2ImplBase{
	...
	@Override
	public void service2Length(RequestLengthMessage request, StreamObserver<ResponseLengthMessage> responseObserver) {
		//prepare the value to be set back
		int length = request.getText().length();
		
		//preparing the response message
		ResponseLengthMessage reply = ResponseLengthMessage.newBuilder().setLength(length).build();
