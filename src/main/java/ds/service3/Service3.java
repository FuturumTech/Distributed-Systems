...
public class Service3 extends Service3ImplBase {
	...
	public void entersToToilet(ToiletVisitsRequest request, StreamObserver<ToiletVisitsResponse> responseObserver) {
		...
		try {
			...
			Toilet.Builder ToiletReply = Toilet.newBuilder()
					.setNumberOfVisits(toiletInDataBase.getNumberOfVisits())
			...
			// preparing the response message, -999 means to the client that unusual path reached
			// We need to use builder to create instance of Toilet that will be passedL
	...
	@Override
	public void updateToiletStatus(UpdateToiletStatusRequest request, StreamObserver<UpdateToiletStatusResponse> responseObserver) {
		...
		try {
			// result of the below method may be null if not found, therefore try catch
			Service3DataBase.Toilet toiletInDataBase = findToilet(toiletName, myTempData.getMyToilets());
				// storing data from request into database anyway, reseting value of visits as the toilet was cleaned:
			...
			//additional check:
			if(toiletInDataBase.getNumberOfVisits() == 0) statusUpdated = true;
			
			reply = UpdateToiletStatusResponse.newBuilder()
					.setStatusUpdated(statusUpdated)
					.setToilet(ToiletReply)
					.setDate(toiletCleanedDateAndTime)
					.build();
